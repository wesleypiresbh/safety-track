...
export default function OrcamentosPage({ clients, vehicles, services, initialOrcamentos }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('/api/orcamentos/add', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(data.message || 'Failed to add orcamento.');
      }
      ...
      // Refresh orcamentos list
      const refreshResponse = await fetch('/api/orcamentos/data');
      const refreshedData = await refreshResponse.json();
      setOrcamentos(refreshedData.orcamentos);
      ...
      if (error instanceof z.ZodError) {
        ...
        setMessage(`Erro ao cadastrar orçamento: ${error.message}`);
      }
    }
  };
  ...
  const handleViewDetails = (orcamentoId) => {
  ...
  const columns = [
    { Header: 'Nº Orçamento', accessor: 'numero_orcamento' },
    { Header: 'Cliente', accessor: 'client_name' },
    { Header: 'Veículo', accessor: 'vehicle_model' },
    { Header: 'Placa', accessor: 'vehicle_plate' },
    { Header: 'Data', accessor: 'data_orcamento' },
    {
      Header: 'Status',
      accessor: 'status',
      Cell: ({ value }) => {
    ...
    { Header: 'Valor', accessor: 'valorOrcamento', Cell: ({ value }) => formatCurrency(value) },
    {
      Header: 'Ações',
      accessor: 'id',
      Cell: ({ row }) => (
  ...
  const formattedOrcamentos = orcamentos.map(orcamento => {
    const datePart = orcamento.data_orcamento.split('T')[0];
    const [year, month, day] = datePart.split('-');
    return {
        ...orcamento,
        data_orcamento: `${day}/${month}/${year}`,
    };
  });
  ...
  return (
    <Layout>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold">Painel de Orçamentos</h1>
      ...
      <Table columns={columns} data={formattedOrcamentos} />
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <FormInput type="text" name="numeroOrcamento" placeholder="Gerado Automaticamente" readOnly disabled />
            </div>
            <div>
...
export async function getServerSideProps(context) {
  ...
  try {
    ...
    const apiUrl = `${protocol}://${host}/api/orcamentos/data`;
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch orcamentos data: ${response.statusText}`);
    }

    const { clients, vehicles, services, orcamentos } = await response.json();

    const orcamentosWithTotal = orcamentos.map(orcamento => {
      let total = 0;
      ...
      if (orcamento.pecas && orcamento.pecas.length > 0) {
        orcamento.pecas.forEach(peca => {
          if (peca.value) {
            total += parseFloat(peca.value);
          }
        });
      }
      return { ...orcamento, valorOrcamento: total };
    });
    ...
    return {
      props: {
        ...
        vehicles,
        services,
        initialOrcamentos: orcamentosWithTotal, // Pass orcamentos with calculated total
      },
    };
  } catch (error) {
